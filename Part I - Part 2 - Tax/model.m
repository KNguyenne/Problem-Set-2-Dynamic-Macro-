...
classdef model
    methods(Static)
        ...
        function par = setup()            
            ...
            %% Load age‐specific tax scale G_t
            T_data = readtable('Tt_by_age.csv');    
            par.Tax = T_data.Tt; 
        ...
        function par = gen_grids(par)
            ...
            par.amax = 300.0; % Upper bound for a.
        ...
        function u = utility(c, gt, par)
    % CRRA utility with government spending benefit
    if par.gamma == 1
        u = log(c); % Log utility
    else
        u = (c.^(1 - par.gamma)) / (1 - par.gamma) + gt; % CRRA utility
    end
        end
